...
<td class="td2 tebal nama_s" no-surah="${d.number}">${d.englishNameTranslation}</td>
 <td class="td2 tebal nama_s" no-surah="${d.number}">${d.revelationType}</td>             
...
const url_api = `https://api.alquran.cloud/v1/surah/${no_s}/editions/ar.alafasy,en.transliteration,en.arberry`;
  let surah = [];
  let jmlAyah;
    ...
    surah.forEach((ayah) => {
      const elementList =
        `<li class="list-item" id="${i}">
          ...
          <br>
 <div class="teks-latin">${ayah.latin}</div>
