function showSurah(data) {
    ...
    data.forEach(element => {
        ...
        <div class="d-flex w-100 justify-content-between">
          ...
          <small   class="text-muted"> <span style="margin-left: 350px;" class="badge bg-success badge-pill ">${element.number}</span> Number Of Ayahs : ${element.numberOfAyahs}</small>
          

<h5 style="text-decoration: #e60e23 underline;font-family: 'Mada', sans-serif;font-size:24px;margin-top:10px;" class="mb-1 Quran-surha">${element.name}</h5>
        </div>
        <p style="text-decoration: #e60e23 underline;margin-top:-34px;font-weight:700;font-family: 'Righteous', cursive;font-size:24px;" class="mb-1 surhaEnglishName">${element.englishName}</p>
        <small style="margin-left: 350px;" class="text-muted">English Name : ${element.englishNameTranslation}</small>
        </button>`
    })
    if (ListOfSurah) {
    ...
    audio_data.ayahs.forEach((element, index,array) => {
       // console.log(element)
        ...
        <div class="card-header">
            <a  style="text-decoration: #e60e23 underline;background-color:#f3ab65;color:#031634;font-family: 'Mada', sans-serif;font-size:14px;" href="#ayah${index + 1}" id="ayahsNo${index}"  class="card-title btn btn-outline-secondary" >- الاية - ${index + 1}</a> 
            </div>
            <div class="card-body bg-light" id='ayah${index + 1}'>
          


  <h5 style=" line-height : 75px;font-family: 'Amiri', serif;font-size:32px;font-weight:700;" class="card-title Quran-ayah " id="text-${index}">${element.text}</h5>
    <hr style=" color:#031634; height: 2px;background: #031634;">  </hr>        
<p style="font-size:20px;font-family: 'Righteous', cursive;" class="card-title Quran-ayah " >${element.en_translation}</p>
...
`
    ...
    document.getElementById(`text-${i}`).style.color = '#e60e23'
